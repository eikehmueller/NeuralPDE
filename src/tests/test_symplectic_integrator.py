...
class QuadraticHamiltonian(Hamiltonian):
    ...
    def H_q(self, q, xi):
        ...
        :arg xi: ancillary vector, d_ancil-dimensional
        """
        return 1 / 2 * torch.einsum("...i,ij,...j->...", q, self.V, q).unsqueeze(dim=-1)
    ...
    def H_p(self, p, xi):
        ...
        :arg xi: ancillary vector, d_ancil-dimensional
        """
        return 1 / 2 * torch.einsum("...i,ij,...j->...", p, self.M, p).unsqueeze(dim=-1)
    ...
    def exact_solution(self, x_0, t):
        ...
        if x_0.ndim == 2:
            A = A.reshape([1, self.d_lat, self.d_lat])
            t = t.reshape(t.shape + (1, 1)).numpy()
...
def test_accuracy(rng):
    ...
    dt = 0.001
    t_final = rng.uniform(low=3, high=4)
    hamiltonian = QuadraticHamiltonian(d_lat, d_ancil, rng, dtype=torch.double)
    X_0 = torch.tensor(rng.normal(0, 1, size=(d_lat + d_ancil,)))
...
def test_convergence(rng):
    ...
    t_final = torch.tensor(rng.uniform(low=3, high=4, size=batch_size))
    hamiltonian = QuadraticHamiltonian(d_lat, d_ancil, rng, dtype=torch.double)
    X_0 = torch.tensor(rng.normal(0, 1, size=(batch_size, d_lat + d_ancil)))
