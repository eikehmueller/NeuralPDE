...
class NeuralPDEModel(torch.nn.Module):
    ...
    def setup(
        ...
        self.x_mean = self.mean[:n_func_in_dynamic + n_func_in_ancillary].unsqueeze(0).unsqueeze(2).to(torch.float32)
        self.y_mean = self.mean[:n_func_in_dynamic].unsqueeze(0).unsqueeze(2).to(torch.float32)
        self.x_std = self.std[:n_func_in_dynamic + n_func_in_ancillary].unsqueeze(0).unsqueeze(2).to(torch.float32)
        self.y_std = self.std[:n_func_in_dynamic].unsqueeze(0).unsqueeze(2).to(torch.float32)
